...
- Repositorios: `src/lib/repositories/units.ts` (patrón base; `save` es **getOrCreate por nombre** — case-insensitive, así dos filas "Gram" nunca coexisten — y `ensureSeeds` siembra las unidades de `docs/database.md`, hace **backfill** de las nuevas por nombre sin tocar unidades propias, y **deduplica** filas legadas duplicadas re-apuntando los productos que las referenciaban + limpia valores de prueba tipo "test"), `products.ts` (`getListItems`/`getDetail` resuelven marca, categoría (incluye su `icon`) y unidad; `save`/`update`/`softDelete`), `categories.ts` (+ seeds con `icon`) y `brands.ts` (`getOrCreateByName`); el Epic 1 extiende el patrón al resto de las entidades. `templates.ts` (SDD 07): `getAll`/`getDetail` agregan items y totales, `save`/`update` reemplazan la lista completa de items (sin diffs, last-write-wins) y `softDelete` borra el template + sus items en una transacción. `daily-log.ts` (SDD 08): `getToday` agrupa los items del día en comidas (`meal_id`) con nombre e ícono derivados por franja horaria local (`mealKind`), `saveMeal` hace upsert del log del día e inserta items con **snapshot nutricional** (el historial queda congelado aunque el producto cambie), `getMeal` resuelve una comida completa para `/meals/[id]`, `updateMealTime` mueve sus items al `daily_logs` del día destino (upsert) con el nuevo timestamp local — el `meal_id` nunca cambia — y `updateItem`/`deleteItem`/`deleteMeal` editan cantidades (re-snapshot) o hacen soft-delete de items/comida entera (SDD 09).
- Catálogo inicial: `src/lib/seeder.ts` — `ensureUserCatalog(userId)` siembra por usuario (categorías, unidades, marcas y productos en orden FK) desde el dashboard, la lista de productos y "Nueva comida"; idempotente (los productos se matchean por nombre incluyendo soft-deletes, así lo borrado no vuelve). Si el usuario no tiene plantillas, además siembra una de arranque — **Desayuno** (3 huevos) — referenciando solo productos existentes.
- Meta diaria de proteína: `DAILY_PROTEIN_GOAL_GRAMS` en `src/lib/daily-goal.ts` (hardcodeada en 150 g; la UI solo lee esa constante, así un futuro settings la reemplaza sin tocar pantallas).
- Matemática de porciones: `src/lib/portion.ts` — **fuente única** de conversión cantidad→porciones. Los macros del producto describen la porción de referencia (`serving_amount` en su unidad, ej. "2 rebanadas"); el usuario entra cantidades en la unidad propia del producto (2 rebanadas, 250 g, 0,5 unidad), así que todo escala por `servingsFor(quantity) = quantity / serving_amount`. `weightGramsFor` convierte a gramos equivalentes y `macroForQuantity` escala un macro por porción. **Nunca** re-implementar esa razón en repositorios, hooks o componentes (el bug clásico: 2 rebanadas de un producto cuya porción es "2 rebanadas" contaba 2 porciones en vez de 1).
- Números para la UI: `src/lib/format.ts` — `formatNumber(value)` con separador decimal de coma, sin ceros de cola y máx. 2 decimales; mata el ruido de floats (40.40000000000006 → "40,4"). Todo número visible pasa por acá.
- Draft de editores: `src/lib/use-draft-items.ts` — estado compartido del "carrito" (items + cantidades + totales) que usan el editor de plantillas y el de comidas; la persistencia y los snapshots viven en los repositorios. Exporta `pluralUnit` ("unidad" → "unidades").
...
- Formulario compartido: `src/components/products/product-form.tsx` usado por creación (`/products/new`) y edición (`/products/[id]/edit`). Los tres selects del formulario (marca, categoría, unidad) son **bottom drawers** — el select nativo es feo en Android y no se puede estilizar bien: `BrandDrawer` (lista + crear marca nueva), `CategoryDrawer` (grilla de tiles con el ícono de la categoría) y `UnitDrawer` (lista con las unidades canónicas primero, `UNIT_PICKER_ORDER`). El `AddItemSheet` (cantidad) muestra un caption dinámico por cantidad: "por 1 unidad (60 g)", "por 2 rebanadas (50 g)", "por 250 g" — con el peso equivalente calculado y plurales correctos.
...
- `AppShell` (`src/components/shell/`): header fijo (hamburguesa o back arrow en pantallas de tarea), bottom nav con 3 tabs (Inicio, Plantillas, Productos) y FAB contextual configurado por ruta.
